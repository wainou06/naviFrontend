...
import { deleteUserInfo, getUserInfo, suspendUserInfo, userPasswordEdit } from '../api/infoApi'
...
const slice = createSlice({
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(userPasswordEditThunk.pending, (state) => {
            state.loading = true
            state.error = null
         })
         .addCase(userPasswordEditThunk.fulfilled, (state) => {
            state.loading = false
            state.error = null
         })
         .addCase(userPasswordEditThunk.rejected, (state, action) => {
            state.loading = false
            state.error = action.payload
         })
   },
